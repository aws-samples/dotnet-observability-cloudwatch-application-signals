...
# New function to delete load balancers
delete_load_balancers() {
    log INFO "Removing AWS load balancers..."
    
    # Get all load balancers
    local lbs=$(aws elbv2 describe-load-balancers --region "${AWS_REGION}" --query 'LoadBalancers[*].LoadBalancerArn' --output text)
    
    for lb in $lbs; do
        # Check if LB belongs to our cluster
        if aws elbv2 describe-tags --resource-arns "$lb" --region "${AWS_REGION}" \
    ...
    log SUCCESS "Load balancers removed"
}
...
main() {
    ...
    delete_alb_controller
    delete_load_balancers  # New step added here
    delete_cert_manager
    delete_cloudwatch_resources
    delete_cloudwatch_logs
    delete_service_accounts
