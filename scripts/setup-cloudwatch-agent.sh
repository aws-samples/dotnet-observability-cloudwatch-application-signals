...
setup_cloudwatch_addon() {
    ...
    if [[ "$addon_status" == "NOT_FOUND" ]]; then
        ...
        local update_available=$(aws eks describe-addon-versions --addon-name "$addon_name" --kubernetes-version $(kubectl version --short | grep 'Server Version:' | cut -d ' ' -f3) --query 'addons[0].addonVersions[0].compatibilities[0].defaultVersion' --output text)
...
main() {
    ...
    log INFO "\nSetup includes:"
    log INFO "- Application Signals discovery enabled"
    log INFO "- CloudWatch service account with IAM roles"
    log INFO "- Amazon CloudWatch Observability EKS add-on"
    log INFO "\nFeatures enabled:"
    log INFO "1. Enhanced Container Insights"
    log INFO "2. CloudWatch Application Signals"
    log INFO "3. Container logs via Fluent Bit"
